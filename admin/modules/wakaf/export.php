<?php
/**
 * Copyright (C) 2021  Ibrahim (islahboim@gmail.com)
 *
...
if (isset($_POST['doExport'])) {
  ...
  if (empty($_POST['fieldSep']) OR empty($_POST['fieldEnc'])) {
    ...
    $sql ="SELECT * FROM wakif ORDER BY input_date ASC";
    ...
    if ($dbs->error) {
      utility::jsAlert(__('Error on query to database, Export FAILED!'));
        ...
        if ($all_data_q->num_rows > 0) {
          ...
          header('Content-Disposition: attachment; filename="wakaf_export.csv"');
          while ($wakaf_d = $all_data_q->fetch_row()) {
              $buffer = null;
              foreach ($wakaf_d as $idx => $fld_d) {
                  // skip wakif_id field
                  if ($idx > 0) {
...
<div class="menuBoxInner exportIcon">
	...
	<div class="infoBox">
    	<?php echo __('Export wakaf data to CSV file'); ?>
	</div>
</div>
</fieldset>
<?php
