...
if (isset($_SESSION['memberID'])) {
    ...
    while ($loan_list_data = $loan_list_query->fetch_assoc()) {
        ...
        // return link
        $return_link = '<a href="#" onclick="confirmProcess('.$loan_list_data['loan_id'].', \''.$loan_list_data['item_code'].'\', \'return\')" title="'.__('Return this item').'" class="btn btn-primary btn-sm"><i class="fas fa-undo-alt"></i> '.__('Return').'</a>';
        // extend link
        ...
        if ($_SESSION['is_expire']) {
            ...
            if ($loan_list_data['return_date'] == date('Y-m-d') || in_array($loan_list_data['loan_id'], $_SESSION['reborrowed']) || $loan_list_data['extend'] == 1) {
                ...
                $extend_link = '<a href="#" onclick="confirmProcess('.$loan_list_data['loan_id'].', \''.$loan_list_data['item_code'].'\', \'extend\')" title="'.__('Extend loan for this item').'" class="btn btn-success btn-sm"><i class="fas fa-clock"></i> <span>'.__('Extend').'</span></a>';
            }
        }
        // renewed flag
    ...
    if ($is_overdue) {
        echo '  <div class="alert alert-danger p-2 rounded-0">
                    <div class="row">
                        <div class="pt-2 col"><strong><i class="fas fa-exclamation-circle"></i> '.__('Total amount of fine is').' '.currency($_total_temp_fines).'</strong></div>
                        <div class="col text-right"><a class="btn btn-danger sendEmail usingAJAX" href="'.MWB.'membership/overdue_mail.php'.'" postdata="memberID='.$memberID.'" loadcontainer="emailStatus"><i class="fas fa-envelope"></i> '.__('Send overdues notice e-mail').'</a></div>
                    </div>
                </div>
