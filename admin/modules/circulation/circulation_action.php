...
if (isset($_POST['memberID']) OR isset($_SESSION['memberID'])) {
    ...
    if (!$member->valid()) {
        ...
        // show the workspace with member information
        echo '<div class="circulation-workspace">';

        // Workspace Hero Header
        echo '<div class="workspace-hero">';
        echo '<div class="workspace-hero__icon"><i class="fas fa-exchange-alt"></i></div>';
        echo '<div class="workspace-hero__text">';
        echo '<h2>'.__('Active Transaction').'</h2>';
        echo '<p>'.__('Member').': <strong>'.$member->member_name.'</strong> ('.$member->member_id.')</p>';
        echo '</div>';

        // Stats
        echo '<div class="workspace-hero__stats">';

        // Count current loans
        $current_loans = count($_SESSION['temp_loan']);
        echo '<div class="workspace-stat">';
        echo '<div class="workspace-stat__value">'.$current_loans.'</div>';
        echo '<div class="workspace-stat__label">'.__('Items').'</div>';
        echo '</div>';

        // Get member's total active loans
        $total_loans_q = $dbs->query("SELECT COUNT(*) FROM loan WHERE member_id='".$dbs->escape_string($member->member_id)."' AND is_lent=1 AND is_return=0");
        $total_loans = $total_loans_q->fetch_row()[0];
        echo '<div class="workspace-stat">';
        echo '<div class="workspace-stat__value">'.$total_loans.'</div>';
        echo '<div class="workspace-stat__label">'.__('Loans').'</div>';
        echo '</div>';

        echo '</div>'; // workspace-hero__stats
        echo '</div>'; // workspace-hero

        // Workspace Surface
        echo '<div class="workspace-surface">';

        // Finish button
        echo '<div style="margin-bottom: 20px; padding-bottom: 16px; border-bottom: 2px solid var(--bg-lighter);">';
        echo '<form id="finishForm" method="post" target="blindSubmit" action="'.MWB.'circulation/circulation_action.php">';
        echo '<button type="button" class="workspace-btn workspace-btn--primary" style="background: var(--danger-gradient);" id="circFinish" accesskey="T" onclick="confSubmit(\'finishForm\', \''.__('Are you sure want to finish current transaction?').'\', '.config('enable_chbox_confirm', '1').')">';
        echo '<i class="fas fa-check-circle"></i> <span>'.__('Finish Transaction').' (Esc)</span>';
        echo '</button>';
        echo '<input type="hidden" name="finish" value="true" />';
        echo '</form>';
        echo '</div>';

        // Member Information Section
        echo '<div class="workspace-member-header">';
        echo '<h3><i class="fas fa-user-circle"></i> '.__('Member Information').'</h3>';
        echo '</div>';
        ...
        echo '<a class="nav-item nav-link '.$active_loan.' notAJAX" id="circLoan" href="'.MWB.'circulation/loan.php" target="listsFrame"><i class="fas fa-hand-holding"></i> '.__('Loans').' (F2)</a>';
        echo '<a class="nav-item nav-link '.$active_loan_list.' notAJAX" id="circInLoan" href="'.MWB.'circulation/loan_list.php" target="listsFrame"><i class="fas fa-list-ul"></i> '.__('Current Loans').' (F3)</a>';
        if ($member_type_d['enable_reserve']) {
          echo '<a class="nav-item nav-link notAJAX" id="circReserve" href="'.MWB.'circulation/reserve_list.php" target="listsFrame"><i class="fas fa-bookmark"></i> '.__('Reserve').' (F4)</a>';
        }
        if ($fines_alert) {
            echo '<a class="nav-item nav-link notAJAX" id="circFine" href="'.MWB.'circulation/fines_list.php" target="listsFrame"><strong class="text-danger"><i class="fas fa-exclamation-circle"></i> '.__('Fines').' (F9)</strong></a>';
        } else {
            echo '<a class="nav-item nav-link notAJAX" id="circFine" href="'.MWB.'circulation/fines_list.php" target="listsFrame"><i class="fas fa-dollar-sign"></i> '.__('Fines').' (F9)</a>';
        }
        echo '<a class="nav-item nav-link notAJAX" id="circHistory" href="'.MWB.'circulation/member_loan_hist.php" target="listsFrame"><i class="fas fa-history"></i> '.__('Loan History').' (F10)</a>'."\n";
        echo '</div>';
        echo '<iframe src="'.$iframe_src.'" id="listsFrame" name="listsFrame" class="s-iframe expandable"></iframe>'."\n";

        // Close workspace divs
        echo '</div>'; // workspace-surface
        echo '</div>'; // circulation-workspace
    }
