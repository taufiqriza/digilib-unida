...
if (isset($_SESSION['memberID'])) {
    ...
    <div class="s-circulation__loan loanItemCodeInput">
        <form name="itemLoan" id="loanForm" action="circulation_action.php" method="post" class="form-inline">
            <label style="font-weight: 600; color: var(--text-dark);"><i class="fas fa-book"></i> <?php echo __('Insert Item Code/Barcode'); ?></label>
            <input type="text" id="tempLoanID" name="tempLoanID" onKeyUp="triggerKeys()" class="form-control col-md-3" placeholder="Scan or type barcode..." />
            <input type="submit" value="<?php echo __('Loan'); ?>" id="executeLoan" class="s-btn btn btn-primary" />
            <?php if($sysconf['barcode_reader']) : ?>
            <a class="s-btn btn btn-info notAJAX" id="barcodeReader" href="<?php echo MWB.'circulation/barcode_reader.php?mode=circulation' ?>"><i class="fas fa-barcode"></i> Open Barcode Reader - Experimental (F8)</a>
            <?php endif ?>
        </form>
    </div>
    ...
    if (count($_SESSION['temp_loan']) > 0) {
        ...
        foreach ($_SESSION['temp_loan'] as $_loan_ID => $temp_loan_list_d) {
            ...
            // remove link
            $remove_link = '<a href="circulation_action.php?removeID='.$temp_loan_list_d['item_code'].'" title="'.__('Remove this item').'" class="btn btn-danger btn-sm"><i class="fas fa-times"></i> <span>'.__('Remove').'</span></a>';
