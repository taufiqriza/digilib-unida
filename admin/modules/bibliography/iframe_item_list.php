...
if ($biblioID) {
  ...
  <div class="biblio-table-wrapper">
    <div class="biblio-table-card">
      <table class="s-table table" style="width:100%">
        <thead>
          <tr>
            <th><?php echo __('Item Code'); ?></th>
            <th><?php echo __('Location'); ?></th>
            <th><?php echo __('Shelf Location'); ?></th>
            <th><?php echo __('Collection Type'); ?></th>
  ...
  if($item_q->num_rows > 0){
    ...
    while ($item_d = $item_q->fetch_assoc()) {
      // links
      $edit_link = '<a class="s-btn btn btn-default notAJAX openPopUp" href="'.MWB.'bibliography/pop_item.php?inPopUp=true&action=detail&biblioID='.$biblioID.'&itemID='.$item_d['item_id'].'" width="650" height="400" title="'.__('Edit Item').'">' . __('Edit') . '</a>';
      $remove_link = '<a class="s-btn btn btn-danger notAJAX" href="#" onclick="javascript: confirmProcess('.$biblioID.', '.$item_d['item_id'].')" title="'.__('Delete Item').'">' . __('Delete') . '</a>';
      ?>
      <tr>
        <td><?php echo $item_d['item_code']; ?></td>
        <td><?php echo $item_d['location_name']; ?></td>
        <td><?php echo $item_d['site']; ?></td>
        <td><?php echo $item_d['coll_type_name']; ?></td>
        <td><?php echo $item_d['item_status_name']; ?></td>
    ...
    echo '<tr><td colspan="6" class="text-center">'.__('No item data available').'</td></tr>';
  }
  ?>
        </tbody>
      </table>
