...
// ensure configured template exists; fallback to default if missing
$templateDir = $sysconf['template']['dir'] ?? 'template';
$templateTheme = $sysconf['template']['theme'] ?? 'default';
if (!is_dir($templateDir . '/' . $templateTheme)) {
    $sysconf['template']['theme'] = 'default';
    $sysconf['template']['css'] = $templateDir . '/default/style.css';
}
