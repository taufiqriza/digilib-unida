...
require_once LIB . 'Branch/BranchManager.php';
require_once LIB . 'Branch/helpers.php';

// Initialize branch manager and detect current branch
$branchManager = \SLiMS\Branch\BranchManager::getInstance($dbs);
$currentBranch = $branchManager->detect();

// Define current branch ID as constant for easy access
define('CURRENT_BRANCH_ID', \SLiMS\Branch\BranchManager::getCurrentBranchId());

// Override library name with branch name if available
if ($currentBranch && !empty($currentBranch['branch_name'])) {
    $sysconf['library_name'] = $currentBranch['branch_name'];
}
if ($currentBranch && !empty($currentBranch['branch_address'])) {
    $sysconf['library_address'] = $currentBranch['branch_address'];
}
