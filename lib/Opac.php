...
class Opac
{
    ...
    public function parseToTemplate()
    {
        ...
        // parse into template
        $templateDir = $opac->sysconf['template']['dir'] ?? 'template';
        $templateTheme = $opac->sysconf['template']['theme'] ?? 'default';
        $templatePath = $templateDir . '/' . $templateTheme . '/index_template.inc.php';

        if (!is_readable($templatePath)) {
            $fallbackTheme = 'default';
            $fallbackPath = $templateDir . '/' . $fallbackTheme . '/index_template.inc.php';
            if (is_readable($fallbackPath)) {
                $opac->sysconf['template']['theme'] = $fallbackTheme;
                $opac->sysconf['template']['css'] = $templateDir . '/' . $fallbackTheme . '/style.css';
                if (isset($opac->definedVariable['sysconf']) && is_array($opac->definedVariable['sysconf'])) {
                    $opac->definedVariable['sysconf']['template']['theme'] = $fallbackTheme;
                    $opac->definedVariable['sysconf']['template']['css'] = $templateDir . '/' . $fallbackTheme . '/style.css';
                }
                $sysconf['template']['theme'] = $fallbackTheme;
                $sysconf['template']['css'] = $templateDir . '/' . $fallbackTheme . '/style.css';
